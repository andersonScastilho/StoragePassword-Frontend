    ...
    <header className="bg-fuchsia-100 h-16 flex items-center p-4 justify-between">
      <h1 className="text-texto-principal font-semibold text-[1.2rem]">
        ...
        <div className="flex gap-8">
            <p>{<HiHome onClick={() => navigateTo('/')} cursor={'pointer'} size={30}/>}</p>
          {isAuthenticated && (
            <p>{<PiPersonSimpleRunFill cursor={'pointer'} size={30} />}</p>
          )}

          {!isAuthenticated && (
            <p>{<SlLogin  onClick={() => navigateTo('/sign-in')}  cursor={'pointer'}  size={30} />}</p>
          )}

          {!isAuthenticated && (
            <p>{<AiOutlineUserAdd  onClick={() => navigateTo('/sign-up')} cursor={'pointer'} size={30}/>}</p>
          )}
